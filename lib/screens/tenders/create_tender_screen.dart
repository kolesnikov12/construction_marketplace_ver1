// lib/screens/tenders/create_tender_screen.dart
import 'dart:io';
...
class _CreateTenderScreenState extends State<CreateTenderScreen> {
  ...
  SnackBar(content: Text(AppLocalizations.of(context)!.translate('tender_items_required'))),
  );
  return;
  }
  ...
  try {
  final tenderItems = _items.map((item) => TenderItem(
...
void _addItem() {
  setState(() {
    if (_items.length < 30) {  // Maximum 30 items allowed
      _items.add({});
    }
  });
}
...
Future<void> _pickFiles() async {
  try {
    FilePickerResult? result = await FilePicker.platform.pickFiles(
      ...
      allowedExtensions: ['jpg', 'jpeg', 'png', 'pdf', 'xlsx', 'xls', 'doc', 'docx', 'zip', 'rar'],
      allowMultiple: true,
    );
    ...
    if (result != null) {
      ...
      if (_selectedFiles.length + files.length > 5) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(AppLocalizations.of(context)!.translate('max_5_files'))),
        );
        return;
      }
